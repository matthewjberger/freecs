...
- `raw_storage` (off by default, implies `dynamic`): the maximum-speed backend for the dynamic world. It swaps component columns from `Box<dyn Any>` + `Vec<T>` to a contiguous byte buffer read through pointer casts (dropping the per-access downcast), recycles freed column allocations through a thread-local buffer pool (dropping the alloc/dealloc churn on repeated spawn/despawn), and disables per-row change detection (dropping the tick bookkeeping that every mutable access and every migration otherwise pays). The **public API is byte-for-byte identical** to the safe backend. Observable behavior is identical too, with one deliberate exception: `changed::<T>()`, `added::<T>()`, and the `for_each_mut_changed` family match nothing, because the tick columns they read are never materialized; `HierarchyIndex` stays correct by rescanning. Every `unsafe` is confined to one `RawColumn` type and verified with `miri`. Leave it off to keep the crate provably `unsafe`-free and to keep change detection; turn it on when you want maximum throughput and do not depend on change/added filters.
