...
#[derive(Default, Debug, Clone, Copy)]
struct Paddle { width: f32, height: f32, speed: f32, is_left: bool }

#[derive(Default, Debug, Clone, Copy)]
struct Ball { radius: f32 }
...
async fn main() {
    ...
    let left_paddle = EntityBuilder::new()
        ...
        .with_paddle(Paddle { width: 20.0, height: 100.0, speed: 400.0, is_left: true })
        .spawn(&mut world, 1)[0];
    
    let right_paddle = EntityBuilder::new()
        .with_position(Position { x: 750.0, y: 300.0 })
        .with_paddle(Paddle { width: 20.0, height: 100.0, speed: 400.0, is_left: false })
        .spawn(&mut world, 1)[0];
...
fn paddle_input(world: &mut World, paddle: Entity, up_key: KeyCode, down_key: KeyCode) {
    ...
    let Some(pos) = world.get_position_mut(paddle) else { return };
...
fn ai_paddle(world: &mut World, paddle: Entity, ball: Entity) {
    ...
    let Some(paddle_pos) = world.get_position_mut(paddle) else { return };
...
fn render(world: &World) {
    for entity in world.query_entities(POSITION | PADDLE) {
        let pos = world.get_position(entity).unwrap();
        let paddle = world.get_paddle(entity).unwrap();
        draw_rectangle(
    ...
    for entity in world.query_entities(POSITION | BALL) {
        let pos = world.get_position(entity).unwrap();
        let ball = world.get_ball(entity).unwrap();
        draw_circle(pos.x, pos.y, ball.radius, WHITE);
    }
    
    draw_line(screen_width() / 2.0, 0.0, screen_width() / 2.0, screen_height(), 2.0, WHITE);
    
    draw_text(&world.resources.score_left.to_string(), screen_width() / 2.0 - 50.0, 50.0, 50.0, WHITE);
    draw_text(&world.resources.score_right.to_string(), screen_width() / 2.0 + 30.0, 50.0, 50.0, WHITE);
}
